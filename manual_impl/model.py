...
class GGNN(nn.Module):
    ...
    def forward(self, features):
        typed_nodes = []
        typed_nodes.append(self.g.src_list[self.g.edge_types])
        typed_nodes.append(self.g.src_list[self.g.edge_types == 0])
        # filter features according to edge type
        typed_features = []
        for i in range(2):
            typed_features.append(features[typed_nodes[i]])
        ...
        scatter_add(typed_msgs[0].t(), typed_nodes[0].unsqueeze(1).expand(typed_msgs[0].shape).t(), out=reduced_msgs)
        reduced_msgs = reduced_msgs + \
            scatter_add(typed_msgs[1].t(), typed_nodes[1].unsqueeze(1).expand(typed_msgs[1].shape).t(), dim_size=self.g.num_nodes)
